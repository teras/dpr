...
template def(typeName:untyped, methodName:untyped, actionArg:string): untyped =
  method methodName(self:typeName, args:seq[string]): void = discard exec(actionArg, args)
...
proc exec(cmd:string,  args:seq[string]): int =
  var a = cmd
  for i in 0..<args.len :
    a.add " "
